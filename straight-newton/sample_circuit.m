...
%Load
sload3=(285*(0.97+j*0.03)+265*(0.85+j*0.15))/Sbase*1000;
sload4=(445*(0.97+j*0.03))/Sbase*1000;
sload5=(750*(0.97+j*0.03))/Sbase*1000;
sload2=(565*(0.97+j*0.03))/Sbase*1000;
sload7=(90*(0.85+j*0.15))/Sbase*1000;
sload8=(605*(0.97+j*0.03))/Sbase*1000;
sload9=(675*(0.85+j*0.15))/Sbase*1000;
sload10=(490*(0.97+j*0.03)+80*(0.85+j*0.15))/Sbase*1000;
sload11=(340*(0.97+j*0.03))/Sbase*1000;
sload13=(40*(0.85+j*0.15))/Sbase*1000;
sload14=(215*(0.97+j*0.03)+390*(0.85+j*0.15))/Sbase*1000;
...
i2=18;
%[x,fval,exitflag]=fsolve(@(x)funFFF(x,Yg,pavsc,sload,uth,zth,k_w,k_pw,w0,k_q),x0,options);
...
[XK,DFk,res,it] = nnewton(x0,tol,itmax,...
    @(x)funFFF(x,Yg,pavsc,sload,uth,zth,k_w,k_pw,w0,k_q),...
    @(x)JfunFFF(x,h,Yg,pavsc,sload,uth,zth,k_w,k_pw,w0,k_q));

%itmax = 50
% for r = 1:3
% [XK,DFk,res,it] = nnewton(XK(:,end),tol,itmax,...
%     @(x)funFFF(x,Yg,pavsc,sload,uth,zth,k_w,k_pw,w0,k_q),...
%     @(x)JfunFFF(x,h,Yg,pavsc,sload,uth,zth,k_w,k_pw,w0,k_q));
% end
% 
% NeWD = @(x) DFk;
% [XK,DFk,res,it] = nnewton(XK(:,end),tol,itmax,...
%     @(x)funFFF(x,Yg,pavsc,sload,uth,zth,k_w,k_pw,w0,k_q),NeWD);

%dat_zsc=dat_zsc.   ';
%save('dat_sweepzsc_bus12_grdiconnected.mat');
